...
## 2026-08-26 — Pilote livré en style P ; les planches de référence deviennent versionnées

Guillaume a demandé le pilote de 70 s en **style P** (le rendu « série animée moderne » de ses trois images d'essai). Bloc de style **récupéré tel quel** sur les générations d'essai et versionné (`build_prompts_pilote.py`) — un style ne se re‑rédige pas de mémoire, il se relit sur la génération qui a été validée. Constat au passage : le dépôt versionnait les briques de plans mais **pas les cinq planches de référence** ; c'est corrigé par `docs/scripts/build_refs_pilote.py`, qui les reconstruit pour n'importe quel style. 18 clés (1 seule reprise : P4a‑1, enveloppe molle) + 18 clips LTX‑2.5 voix libres, **sans re‑rendu de dialogue** parce que la règle « citer les répliques entre guillemets » était déjà écrite dans la stratégie : la leçon de v7 a coûté une heure de pod une fois, zéro la deuxième. Montage 62,8 s livré. **Point ouvert relevé sur le montage** : l'aspect du ballon change entre les plans lointains 4b — à tenir par une planche de référence « ballon » si P est retenu. Le choix du style reste à Guillaume, maintenant entre **A, B, D, J, K et P**.
...
## 2026-08-26 — Style P : les figurants sortaient avec le visage noirci ; RÈGLE 37

Retour de Guillaume sur le montage en style P : « le visage des personnages est tout noir quand ils sont en fond ». Cause trouvée à la source, sur la planche `Foule_StyleP`, puis propagée par réinjection aux sept plans qui l'utilisent (RÈGLE 1). Les styles A et D, avec les mêmes mots, n'ont pas le défaut : demander « aucun trait de visage » à un style qui ne dispose que de deux ou trois tons par zone revient à demander le ton le plus sombre. Correction conforme au corollaire de la RÈGLE 35 : on supprime la **zone** — toutes les têtes strictement de dos — au lieu de supprimer les traits, on nomme la carnation en positif, et la négative ne vise plus que le remplissage. Variante `P` ajoutée aux blocs identité, à côté de `AC`, `B` et `JK`. Coût : 1 planche + 7 clés reprises (16 crédits) et 7 clips à re-rendre. Le ballon, lui, reste dans la même famille sur les reprises : le défaut de continuité relevé la veille n'est pas aggravé.

Deux incidents techniques consignés au passage : l'API S3 de RunPod refuse **`HeadObject`** (403), donc `download_file` de boto3 échoue et il faut lire `get_object` directement ; et **EU-RO-1 n'avait aucun GPU disponible** au moment de relancer les sept clips — le volume et ses 37 Go de modèles y étant, on attend la capacité plutôt que de déménager dans un autre datacenter, ce qui coûterait 45 minutes de retéléchargement.

## 2026-08-26 — Le ballon passe en référence : la continuité tient, mais la réinjection a deux effets de bord

Guillaume : « fais la planche du ballon et refais les plans 4b ». Deux précisions issues du relevé : **le bloc 4b ne montre pas le ballon** — de 45,2 s à 55 s on est dans la nacelle, rebord, main gantée, toits de Paris — et l'écart d'aspect que j'avais signalé la veille est en réalité sur **P4a-3**, à 41-45 s. Les plans qui montrent le ballon sont au nombre de onze : 1a-1 à 1a-4, 1b-1, 1b-2, P02, 4a-1, 4a-2, 4a-3, plus 1b-3 où il est censé être hors champ.
