...
## Pilote en style P (2026-08-25/26, pod A100 `jws14sbc01vm9b`)

Demande de Guillaume : « peux‑tu faire le pilote de 70 secondes en style P ? ». Style P = le rendu **série animée moderne** des trois images d'essai qu'il avait retenues ; il n'existait alors ni bloc de style versionné, ni planches de référence, ni clés.

**Reconstitution du style** : les trois images d'essai retrouvées chez Higgsfield (`show_generation_by_ids`) portent leur prompt complet ; le bloc de style et le bloc d'époque en ont été extraits **mot pour mot** et ajoutés à `STYLE`, `EPOQUE` et `NEG_STYLE` de `docs/scripts/build_prompts_pilote.py` (aucune reformulation, RÈGLE 13). Style P est un style **dessiné**, donc variante d'identité AC (pas inkman).

**Manque comblé au passage** : les 20 briques de plans étaient versionnées, mais **pas les cinq planches de référence** réinjectées ensuite (D01, D02, Foule, Garnerin, Parieurs) — elles n'existaient que dans l'historique des sessions. Écrit `docs/scripts/build_refs_pilote.py`, qui les assemble pour n'importe quel style à partir des mêmes blocs de style/époque/négatives et des blocs identité de la fiche personnages. Les cinq planches P sont générées, contrôlées et renvoyées comme médias (media_id dans la fiche des identifiants).

**18 clés** générées avec les références réinjectées, contrôlées une par une sur planche‑contact contre le script : présence des personnages, intentions, positions. Un seul défaut — **P4a‑1 : enveloppe molle** (ballon déjà dégonflé alors qu'il monte) → reprise, prompt réécrit en entier. Le reste conforme du premier coup, y compris les deux points durs corrigés en v7 : **P5‑1/P5‑2, le bras qui tient le couteau part de l'intérieur de la nacelle** (rebord au premier plan, couteau contre la paroi intérieure) et **P03 construit sur l'action** (Garnerin lâche la corde de largage, l'aide supplie, les deux visages visibles).

**18 clips** rendus en LTX‑2.5 (deux passes, 1280×704, 24 i/s, voix libres générées avec l'image), prompts de dialogue **citant les répliques entre guillemets** — la leçon de v7 appliquée dès le premier rendu, donc **aucun charabia et aucun re‑rendu** : les quatre répliques sortent mot pour mot à la transcription (« Il va se tuer, je vous dis. » / « Dix francs qu'il ne coupe pas la corde. » / « Citoyen Garnerin, renoncez, il est encore temps. » / « Lâchez tout. »). Montage `montages/montage_StyleP_v7.mp4`, **62,8 s**, planche 1 i/s `_thumbs/v7/StyleP_v7_1fps.jpg`.
...
### Trois retours de Guillaume traités ensemble (26 août 2026, pod RTX PRO 6000 `0i7pa2oahbjl21`)

**1. « Une femme met un chapeau alors qu'elle a déjà quelque chose sur la tête. »** Vérifié image par image sur 4a-2 : à 38 s elle porte un bonnet, à 39 s un second chapeau apparaît dans ses mains et vient se poser dessus ; l'homme de gauche fait de même sur son haut-de-forme. Cause : le prompt de mouvement disait `hats held on with both hands` — nommer un objet déjà présent comme complément d'un verbe de possession en fabrique un second exemplaire. **RÈGLE 40** : on décrit un déplacement, jamais une possession ; et la garde `Every object visible is already present in the first frame…` est désormais collée par `build_clips_pilote.py` sur **tous** les prompts de mouvement, pendant exact de `Nobody new enters the frame` posée le 22 août pour les personnes.

**2. « La nacelle n'est pas toujours la même. »** Exact : ronde à rebord épais sur 1b-3, **rectangulaire à couvercle** sur 1b-2, rectangulaire à panneaux sur P03, ronde à bourrelet sur 4b-2. Même cause que le ballon, et Guillaume l'a nommée : les objets de l'épisode n'avaient pas de description assez précise pour la génération d'images. **RÈGLE 38** : un nom commun n'est pas une description ; un asset se décrit en six points — forme, structure, matière, couleurs nommées, détail distinctif, et ce qu'il n'a pas. Les descriptions canoniques des douze objets de continuité de l'épisode sont écrites dans `docs/prompts/S01E01-assets-prompts-v3.4.md` §7. Planche `Nacelle_StyleP` à **deux vues**, extérieure et intérieure — la moitié des plans montrent la nacelle de l'intérieur, une planche extérieure seule n'aurait rien tenu — réinjectée sur les treize plans qui la montrent, avec clause anti second panier.

**3. « À un moment il coupe la corde, mais c'est quelle corde ? »** Le scénario est clair (plan 5 : « la lame tranche la corde qui retient la nacelle au ballon »), le montage ne l'était pas : les trois plans du bloc 5 étaient des très gros plans, aucun ne montrait les deux extrémités de la corde. **RÈGLE 39** : un plan d'action doit tenir dans le même cadre les deux termes de la relation. 5-2 passe en plan moyen serré, rebord en bas, corde qui monte jusqu'au cercle et au ballon ; 5-3 s'élargit pour montrer le bout coupé qui retombe avec la nacelle pendant que le ballon bondit. À l'écran, entre 57 et 62 s, la géométrie se lit maintenant.

**RÈGLE 41**, demandée dans la foulée : toute règle inscrite dans un script est aussi écrite en clair dans le dépôt, avec un tableau de correspondance script ↔ document.
