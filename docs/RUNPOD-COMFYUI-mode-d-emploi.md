...
19. **RTX 5090 (32 Go, 0,99 $/h secure) suffit pour Wan 2.2 A14B fp8 en 720p** : ComfyUI charge un expert à la fois ; 16 sous-clips de 2 à 4 s en 38 min, ~40 à 100 s par clip. Pris faute d'A100 disponible ; pas de différence visible de trait avec l'A100.
20. **Deux images identiques en FLF2V (départ = fin) donnent un clip figé**, même avec un prompt de mouvement explicite : vérifié sur 6 sous-clips de dialogue. Pour la continuité dans un même cadrage, enchaîner par la dernière image rendue (`docs/scripts/chain_dialogue_runpod.py`).
21. **S2V** : `wan2.2_s2v_14B_fp8_scaled.safetensors` (16,4 Go, `models/diffusion_models/`), encodeur audio `wav2vec2_large_english_fp16.safetensors` (`models/audio_encoders/`), LoRA `wan2.2_t2v_lightx2v_4steps_lora_v1.1_high_noise.safetensors` pour 4 étapes cfg 1 (template officiel « video_wan2_2_14B_s2v »). Nœuds `AudioEncoderLoader` → `AudioEncoderEncode(LoadAudio)` → `WanSoundImageToVideo(ref_image, audio_encoder_output, length)` → `KSampler(uni_pc, simple)` ; ModelSamplingSD3 shift 8. Le mp3 se copie dans `input/` du pod (scp), `CreateVideo` prend l'audio. Graphe exécutable : `docs/scripts/run_s2v_runpod.py`. Une réplique ≤ 4,8 s = un rendu de 77 images au plus ; au-delà, le template enchaîne des morceaux (LatentCut / LatentConcat). Bootstrap : `docs/runpod/bootstrap_pod_wan22_s2v.sh`.
